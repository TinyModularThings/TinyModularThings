		...
		if(randomMode)
		{
			...
			if(!ints.isEmpty())
			{
				...
				for(int randSlot : ints)
				{
					...
					boolean flag = sendEnergy(sender, receiver, false);
		...
		if(randomMode)
		{
			...
			if(!ints.isEmpty())
			{
				...
				for(int randSlot : ints)
				{
					...
					boolean flag = sendEnergy(sender, receiver, true);
					if(flag && setMultiPacket)
					{
						continue;
					}
...
//	{
//		int toSend = Math.min(Math.min(sender.getStoredEnergy(), sender.getEnergyToSend()), sender.getTransferlimit());
//		if(toSend <= 0)
//		{
//			return false;
//		}
//		int toReceive = Math.min(Math.min(toSend, receiver.getAviableSpace()), Math.min(receiver.getTransferlimit(), receiver.requestedEnergy()));
//		if(toReceive <= 0)
//		{
//			return false;
//		}
...
//	}
	...
	private boolean sendEnergy(BatteryContainer sender, BatteryContainer receiver, boolean draw)
	{
		if(draw)
		{
			int toDraw = Math.min(sender.getEnergyToSend(), sender.getStoredEnergy());
			if(toDraw <= 0)
			{
				return false;
			}
			int toReceive = Math.min(toDraw, Math.min(receiver.getAviableSpace(), receiver.getTransferlimit()));
			if(toReceive <= 0)
			{
				return false;
			}
		...
		int toReceive = Math.min(toSend, Math.min(receiver.getAviableSpace(), receiver.requestedEnergy()));
		if(toReceive <= 0)
		{
			return false;
		}
	...
	public static class BatteryContainer
	{
		...
		public int getAviableSpace()
		{
			return getMaxStorage() - getStoredEnergy();
		}
		...
		{
			return battery.getMaxTransferlimt(batteryData);
		}
